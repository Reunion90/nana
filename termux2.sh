#!/bin/bash
yes | pkg update
yes | pkg upgrade
yes | pkg install termux-services libjansson wget nano
rm -rf ~/ccminer
rm -rf ~/.ssh
mkdir ~/.ssh
chmod 0700 ~/.ssh
echo "ssh-rsa " > ~/.ssh/authorized_keys
chmod 0600 ~/.ssh/authorized_keys
mkdir ~/ccminer && cd ~/ccminer
wget -q https://raw.githubusercontent.com/btcdollar/Reunion90/main/start.sh -O ~/ccminer/start.sh
wget -q https://raw.githubusercontent.com/btcdollar/Reunion90/refs/heads/main/ccminer_a53 -O ~/ccminer/ccminer
wget -q https://raw.githubusercontent.com/btcdollar/Reunion90/main/config.json -O ~/ccminer/config.json
chmod +x ccminer start.sh
cd && cd && cd
sed -i -e '$acd ccminer/&&./start.sh' ../usr/etc/bash.bashrc
su -c reboot
